...
@section('title', 'Create Mock Topic - Admin Panel')
...
<div class="space-y-6">
        ...
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-600">Add a new topic for mock exam questions</p>
                </div>
    ...
    <!-- Create Topic Form -->
    <form method="POST" action="{{ route('admin.mock-topics.store') }}" class="bg-white shadow rounded-lg">
        @csrf
        <div class="px-6 py-4 border-b border-gray-200">
        ...
        <div class="px-6 py-4 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" required class="block w-full px-3 py-2 border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-colors duration-200 @error('name') border-red-500 @enderror" placeholder="e.g., Algebra">
                    @error('name')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="slug" name="slug" value="{{ old('slug') }}" class="block w-full px-3 py-2 border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-colors duration-200 @error('slug') border-red-500 @enderror" placeholder="auto-generated-from-name">
                    @error('slug')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
