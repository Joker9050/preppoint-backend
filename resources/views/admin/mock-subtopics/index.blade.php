...
@section('title', 'Mock Subtopics - Admin Panel')

@section('breadcrumb')
<li class="flex items-center">
    <i class="fas fa-chevron-right text-gray-400 mx-2"></i>
    <span class="text-gray-500">Mock Subtopics</span>
</li>
@endsection
...
<div class="space-y-6">
    ...
    <div class="bg-white shadow rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Mock Subtopics</h1>
                    <p class="mt-1 text-sm text-gray-600">Manage subtopics for mock exam questions</p>
                </div>
                <div class="flex items-center space-x-3">
                    <a href="{{ route('admin.mock-subtopics.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-lg p-6 text-white">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <dt class="text-sm font-medium opacity-90">Parent Topics</dt>
                            <dd class="text-3xl font-bold">{{ $subtopics->unique('topic_id')->count() }}</dd>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg p-6 text-white">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <dd class="text-3xl font-bold">{{ $subtopics->sum(function($subtopic) { return $subtopic->questions->count(); }) }}</dd>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            ...
            <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4">
                <div class="flex items-center space-x-4">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <h3 class="text-lg font-bold text-gray-900">Subtopics Management</h3>
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    ...
                    <div class="flex items-center space-x-4 text-sm">
                        ...
                        <div class="flex items-center text-gray-600">
                            ...
                            <span>Total: <span class="font-bold text-indigo-600">{{ $subtopics->total() }}</span> subtopics</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table id="subtopics-table" class="w-full min-w-[1200px] text-sm text-left">
                    ...
                    <tr class="bg-white hover:bg-indigo-50/30 transition-colors duration-150 group">
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ml-3">
                                    ...
                                    @if($subtopic->description)
                                        <div class="text-xs text-gray-500 mt-1">{{ Str::limit($subtopic->description, 40) }}</div>
                                    @endif
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                {{ $subtopic->topic ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                {{ $subtopic->topic->name ?? 'N/A' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                {{ $subtopic->topic && $subtopic->topic->subject ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800' }}">
                                {{ $subtopic->topic->subject->name ?? 'N/A' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <span class="text-sm text-gray-600">{{ Str::plural('question', $subtopic->questions->count()) }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <div class="flex flex-col">
                                <span>{{ $subtopic->created_at->format('M j, Y') }}</span>
                                <span class="text-xs text-gray-400">{{ $subtopic->created_at->format('g:i A') }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center justify-end space-x-1">
                                <a href="{{ route('admin.mock-subtopics.edit', $subtopic) }}"
                                ...
                                <form action="{{ route('admin.mock-subtopics.destroy', $subtopic) }}" method="POST" class="inline"
                                      onsubmit="return confirm('Are you sure you want to delete this subtopic? This action cannot be undone.')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-gray-500 text-sm mb-4">Get started by creating your first mock subtopic.</p>
                                <a href="{{ route('admin.mock-subtopics.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors">
...
$(document).ready(function() {
    ...
    const table = $('#subtopics-table').DataTable({
        responsive: {
            details: {
                type: 'column',
                target: 'tr'
        ...
        initComplete: function() {
            ...
            $('.dataTables_filter input').attr('placeholder', 'Search subtopics...');
        ...
        drawCallback: function() {
            ...
            $('#subtopics-table tbody tr').hover(
    ...
    $('#subtopics-table').on('mouseenter', '.group/btn', function() {
        const $this = $(this);
        if (!$this.attr('title')) {
...
#subtopics-table tbody tr:last-child td {
    border-bottom: none;
}
